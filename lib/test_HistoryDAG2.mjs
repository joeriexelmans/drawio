...
function runTest(verbose) {
...

  {
    ...
    const op1 = history1.newOperation({geometry: 1});
    const op2 = history1.newOperation({geometry: 3});
    ...
    history1.receiveOperation(serialize(op4)); // buffered
    history1.receiveOperation(serialize(op3)); // op3 wins from op1 -> op2 and op1 undone

    info("history2 receive...")
    history2.receiveOperation(serialize(op1)); // op1 loses from op3
    history2.receiveOperation(serialize(op2));
