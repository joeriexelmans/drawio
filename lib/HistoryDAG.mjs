  ...
  add(newItem) {
    ...
    const conflictingItem = (() => {
      for (const head of this.heads) {
        for (const concurrentItem of iterConcurrentItems(head, newItem)) {
...
// // The same item may be yielded more than once.
...
// Because the history DAG allows for diamond-like structures, the same item may be yielded more than once. The performance penalty of this behavior is probably smaller than the performance penalty of a mechanism to prevent this.
export function* iterConcurrentItems(existingItem, newItem) {
  if (isAncestor2(existingItem, newItem)) {
    return; // encountered an LCA -> stop recursion
  }
  yield existingItem;
  for (const parent of existingItem.parents) {
    yield* iterConcurrentItems(parent, newItem);
  }
}
